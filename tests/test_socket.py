...
def client_comms_queued(_sockClass, _queueClass, port, payload):
    global startup_lock
    startup_lock.acquire()
    startup_lock.release()
    sock = _queueClass(_sockClass('localhost', port))
    payload.intake=[]
    payload.output=[]
    for trial in range(5):
        payload.output.append(make_random_string())
...
class test(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.queuedscript_path = os.path.join(
            os.path.dirname(
                os.path.dirname(
                    os.path.abspath(__file__)
                )
    ...
    def test_queuedsocket_compilation(self):
        compiled_path = compile(self.queuedscript_path)
        self.assertTrue(compiled_path)
    ...
    def test_server_bind_and_communication_queued(self):
        # warnings.simplefilter('error', ResourceWarning)
        from agutil.io import SocketServer
        from agutil.io import Socket
        ...
        client_payload = lambda x:None
        server_thread = threading.Thread(target=server_comms, args=(ss, server_payload))
        client_thread = threading.Thread(target=client_comms_queued, args=(Socket, QueuedSocket, ss.port, client_payload))
        server_thread.start()
        client_thread.start()
        server_thread.join(10)
        self.assertFalse(server_thread.is_alive())
